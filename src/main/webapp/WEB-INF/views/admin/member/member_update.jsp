					...
					<form role="form" action="/admin/member/update" method="post">
						<div class="row">
							...
							<div class="col-sm-12">
								<!-- text input -->
								<div class="form-group">
									<label>email</label> <input value="${memberVO.email}"
							...
							<div class="col-sm-12">
								<div class="form-group">
									<label>point</label> <input value="${memberVO.point}"
										name="point" type="text" class="form-control" placeholder="0">
								</div>
							</div>
							<div class="form-group">
								<label>enabled</label> <select name="enabled"
									class="form-control">
									<!-- 삼항연산자(?)
                                                 system.out.println if 조건문으로 작성
                                                 => memberVO가 false라면? selected 아니면: ''(공백)
                                                 ==> if (memberVO.enabled == 'false') {
   													 System.out.print("selected");
														}else{
														    System.out.print("");
														} -->
									<option value="0"
										<c:out value="${(memberVO.enabled eq 'false')?('selected'):('')}"/>>false</option>
									<option value="1"
										<c:out value="${(memberVO.enabled eq 'true')?('selected'):('')}"/>>true</option>
								</select>
								<label>level</label> <select name="levels" class="form-control">
									...
									<option value="ROLE_ADMIN" <c:out value="${memberVO.levels eq 'ROLE_ADMIN'?'selected':''}"/>>ROLE_ADMIN</option>
								</select>
								<br>
								<div class="buttons">
